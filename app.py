...
<!-- Primary Meta -->
<meta name="description" content="{{meta_description or 'Civision Society — Official student club of the Department of Civil Engineering, IIUC (International Islamic University Chittagong). Join, log in, register, or pay membership dues online.'}}"/>
<meta name="keywords" content="Civision Society, Civision, IIUC Civil Engineering, IIUC Civil Club, Civil Engineering IIUC, International Islamic University Chittagong Civil, IIUC student club, Civision Society login, civil engineering club Bangladesh, civil club IIUC, IIUC civil cost, Department of Civil Engineering IIUC, engineers institution Bangladesh, institute of engineers Bangladesh, bangladesh civil engineers, IIUC login, civil engineering student club, IEB Bangladesh, IIUC civil department, Civision Society membership, IIUC engineering club, civil engineering Chittagong, civil engineers Bangladesh"/>
<meta name="author" content="Civision Society"/>
<meta name="robots" content="{{meta_robots or 'index, follow, max-snippet:-1, max-image-preview:large'}}"/>
<meta name="googlebot" content="{{meta_robots or 'index, follow'}}"/>
...
<!-- Open Graph -->
<meta property="og:title" content="{{_title or 'Civision Society — IIUC Civil Engineering Club'}}"/>
<meta property="og:description" content="{{meta_description or 'Civision Society — Official student club of the Dept. of Civil Engineering, IIUC. Join us, log in, register, or pay membership dues online.'}}"/>
...
<meta name="twitter:title" content="Civision Society — IIUC Civil Engineering Club"/>
<meta name="twitter:description" content="{{meta_description or 'Official student club of the Dept. of Civil Engineering, IIUC. Connected in Purpose.'}}"/>
...
<script type="application/ld+json">
{
  ...
  "alternateName": ["Civision", "IIUC Civil Engineering Club", "IIUC Civil Club", "Civil Engineering Club IIUC"],
  "description": "Civision Society is the official student club of the Department of Civil Engineering at International Islamic University Chittagong (IIUC). We connect civil engineering students through civic education, professional networking, and purposeful events.",
  ...
  "sameAs": [
    "https://www.facebook.com/profile.php?id=61585468977506&mibextid=rS40aB7S9Ucbxw6v"
  ]
}
</script>
...
<script type="application/ld+json">
{
  ...
  "@type": "WebSite",
  "name": "Civision Society",
  "alternateName": "IIUC Civil Engineering Club — Civision Society",
  "url": "{{request.url_root.rstrip('/')}}",
  "description": "Official website of Civision Society, the civil engineering student club of IIUC. Member login, registration, payment, events, and club information.",
  "inLanguage": "en",
  "keywords": "Civision Society, IIUC civil engineering, civil club, IIUC login, civil engineering Bangladesh, Civision login, bangladesh civil engineers, engineers institution"
}
</script>
...
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "ItemList",
  "name": "Civision Society Main Sections",
  "itemListElement": [
    {"@type":"SiteNavigationElement","position":1,"name":"Home","description":"Civision Society homepage — events, announcements, gallery and club info","url":"{{request.url_root.rstrip('/')}}/"},
    {"@type":"SiteNavigationElement","position":2,"name":"Log In","description":"Member login — IIUC civil engineering club member portal","url":"{{request.url_root.rstrip('/')}}/login"},
    {"@type":"SiteNavigationElement","position":3,"name":"Registration Form","description":"Join Civision Society — IIUC civil engineering student registration","url":"{{request.url_root.rstrip('/')}}/join"},
    {"@type":"SiteNavigationElement","position":4,"name":"Payment","description":"Pay monthly membership dues via bKash or Nagad","url":"{{request.url_root.rstrip('/')}}/payment"},
    {"@type":"SiteNavigationElement","position":5,"name":"Admin","description":"Admin panel — Civision Society management","url":"{{request.url_root.rstrip('/')}}/admin"}
  ]
}
</script>
...
<script type="application/ld+json">
{
  ...
  "@type": "BreadcrumbList",
  "itemListElement": [
    {"@type":"ListItem","position":1,"name":"Civision Society","item":"{{request.url_root.rstrip('/')}}"},
    {"@type":"ListItem","position":2,"name":"{{_title or 'Page'}}","item":"{{request.url}}"}
  ]
}
</script>
...
{% if image_schema %}<script type="application/ld+json">{{image_schema|safe}}</script>{% endif %}
</head>
<body>
<nav>
  <div class="ni">
...
<footer>
<a class="fb-link" href="https://www.facebook.com/profile.php?id=61585468977506&mibextid=rS40aB7S9Ucbxw6v" target="_blank" rel="noopener">Civision Society</a> — "Connected in Purpose"<br/>
...
<div class="card">
  <h2>Gallery</h2>
  <!-- Google reads all images below. JS only controls which page is visible. -->
  {% set per_page = 16 %}
  {% for i in range(0, gallery|length, per_page) %}
  <div class="gg gal-page" id="galp{{loop.index0}}" {% if loop.index0 > 0 %}style="display:none;"{% endif %}>
    {% for img in gallery[i:i+per_page] %}
    <div class="gi" onclick="openLB('{{img.url}}')">
      <img src="{{img.url}}"
           alt="{{img.caption or 'Civision Society — IIUC Civil Engineering Club'}}"
           title="{{img.caption or 'Civision Society Gallery'}}"
           loading="lazy"/>
  ...
  {% endfor %}
  {% if gallery|length > per_page %}
  <div class="gg-nav" style="display:flex;margin-top:14px;">
    <button id="galPrev" onclick="galMove(-1)" disabled>‹</button>
    <span class="gg-pageinfo" id="galInfo">1 / {{((gallery|length - 1) // per_page) + 1}}</span>
    <button id="galNext" onclick="galMove(1)">›</button>
  </div>
  <script>
  (function(){
    var pages=document.querySelectorAll('.gal-page');
    var total=pages.length, cur=0;
    function upd(){
      pages.forEach(function(p,i){p.style.display=i===cur?'grid':'none';});
      document.getElementById('galInfo').textContent=(cur+1)+' / '+total;
      document.getElementById('galPrev').disabled=cur===0;
      document.getElementById('galNext').disabled=cur>=total-1;
    }
    window.galMove=function(d){cur=Math.max(0,Math.min(total-1,cur+d));upd();};
    upd();
  })();
  </script>
  {% endif %}
...
{% endif %}"""
    ...
    base_url = "https://civision-society.onrender.com"
    image_schema = json.dumps({
        "@context": "https://schema.org",
        "@type": "ImageGallery",
        "name": "Civision Society Photo Gallery",
        "description": "Official photo gallery of Civision Society — IIUC Civil Engineering Club",
        "url": base_url,
        "image": [
            {
                "@type": "ImageObject",
                "url": g.get("url",""),
                "name": g.get("caption","") or "Civision Society — IIUC Civil Engineering Club",
                "description": (g.get("caption","") or "Photo from Civision Society, the official student club of the Department of Civil Engineering, IIUC") + " | Civision Society | IIUC Civil Club",
                "contentUrl": g.get("url",""),
                "author": {"@type":"Organization","name":"Civision Society"},
                "copyrightHolder": {"@type":"Organization","name":"Civision Society"}
            }
            for g in gallery if g.get("url")
        ]
    }) if gallery else None
    return render(tmpl, _title="Civision Society — IIUC Civil Engineering Club | Connected in Purpose",
                  ...
                  meta_description="Civision Society — Official student club of the Department of Civil Engineering, IIUC (International Islamic University Chittagong). Join, log in, view events, and pay membership dues. IIUC Civil Club, civil engineering Bangladesh, Civision login.")
...
<div style="max-width:640px;margin:auto;">
  <div class="card"><h2>Membership Application</h2>
    ...
    return render(tmpl, _title="Join Civision Society — IIUC Civil Engineering Club Registration Form",
                  nav_logo=get_nav_logo(),
                  meta_description="Register as a new member of Civision Society — the official civil engineering student club of IIUC (International Islamic University Chittagong). Fill out the membership registration form to join.")
...
<div style="max-width:400px;margin:80px auto;">
  <div class="card" style="text-align:center;">
    ...
    return render(tmpl, _title="Log In — Civision Society | IIUC Civil Engineering Club Member Portal",
                  nav_logo=get_nav_logo(),
                  meta_description="Civision Society member login — IIUC civil engineering club member portal. Log in with your Member ID and password to view your profile, payment status, and club balance.")
...
<div style="max-width:640px;margin:auto;">
  ...
  {% endif %}
    ...
    return render(tmpl, _title="Payment — Civision Society | IIUC Civil Club Membership Dues",
                  q=q, member=member, pay_status=pay_status,
                  month_options=month_options, mfull=MONTHS_FULL, nav_logo=get_nav_logo(),
                  meta_description="Pay your Civision Society monthly membership dues online via bKash or Nagad. IIUC civil engineering club payment portal. Submit your transaction ID for admin verification.")
...
def check_cookie_debug():
    ...
    if mc and ":" in mc:
        try:
            ...
            if doc.exists:
                d = doc.to_dict()
                fs_token = d.get("session_token","")
                result["firestore_token"] = fs_token[:20]+"..." if len(fs_token) > 20 else fs_token
                result["tokens_match"] = (fs_token == token)
                result["member_status"] = d.get("status","")
                result["token_len_cookie"] = len(token)
                result["token_len_firestore"] = len(fs_token)
                result["tokens_exact"] = repr(fs_token) == repr(token)
        except Exception as e:
            result["error"] = str(e)
    # Also test get_logged_in_member directly
    try:
        m = get_logged_in_member()
        result["get_logged_in_result"] = m["id"] if m else "None — returned None"
    except Exception as e:
        result["get_logged_in_result"] = f"ERROR: {str(e)}"
...
def image_sitemap_xml():
    ...
    for img in gallery:
        url = img.get("url","")
        caption = img.get("caption","") or "Civision Society — IIUC Civil Engineering Club"
        if url:
            image_entries += (
                f"<url><loc>{base}/</loc>"
                f"<image:image>"
                f"<image:loc>{url}</image:loc>"
                f"<image:caption>{caption}</image:caption>"
                f"<image:title>{caption} | Civision Society</image:title>"
                f"</image:image></url>"
            )
...
def sitemap_xml():
    ...
    pages = [
        ("/",        "1.0", "daily",   today),
        ("/login",   "0.9", "weekly",  today),
        ("/join",    "0.9", "monthly", today),
        ("/payment", "0.8", "weekly",  today),
        ("/admin",   "0.5", "monthly", today),
    ]
    urls = "".join(
        f"<url><loc>{base}{p}</loc><lastmod>{lm}</lastmod><changefreq>{freq}</changefreq><priority>{pri}</priority></url>"
        for p, pri, freq, lm in pages
    )
