...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;">
    <a href="/admin/export/xlsx" class="btn bgrn bsm">📥 Export Excel</a>
    <a href="/admin/logout" class="btn br bsm">Log Out</a>
  </div>
</div>
<div class="card" style="margin-bottom:20px;background:linear-gradient(135deg,#1a3a55,#2c3e50);color:#fff;">
  <h2 style="color:#fff;border-color:#444;">📤 Import Members from Excel</h2>
  <p style="color:rgba(255,255,255,.75);font-size:.9rem;margin-bottom:14px;">
    ...
    <span style="font-size:.8rem;color:rgba(255,255,255,.5);">Existing records with same ID will be updated. Password = plain text, auto-hashed. Status for payments = Verified / Pending.</span>
  </p>
  <form method="post" action="/admin/import/xlsx" enctype="multipart/form-data">
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      ...
      <button class="btn bg" type="submit"
              onclick="return confirm('This will import/update members from your Excel file. Continue?');">
        📤 Import Now
      </button>
    </div>
  </form>
...
def admin_import_xlsx():
    if "xlsx_file" not in request.files:
        flash("No file selected.","e"); return redirect(url_for("admin_dashboard"))
    f = request.files["xlsx_file"]
    if not f or not f.filename.endswith(".xlsx"):
        flash("Only .xlsx files accepted.","e"); return redirect(url_for("admin_dashboard"))
    try:
        ...
        flash(f"Could not read Excel file: {str(e)}","e"); return redirect(url_for("admin_dashboard"))
