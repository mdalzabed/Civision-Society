...
def member_login():
    ...
    tmpl = r"""
<div class="ph"><h1>Member Login</h1><p>Enter your Member ID and password to view your profile.</p></div>
<div style="max-width:420px;margin:auto;">
  <div class="card">
    ...
    return render(tmpl, _title="Member Login — Civision Society", nav_logo=get_nav_logo())
...
def member_profile():
    ...
    if not member or member.get("status") != "Approved":
        session.pop("member_id", None)
        flash("Session expired. Please log in again.","e")
        return redirect(url_for("member_login"))
    payments = get_payment_status_for_member(mid)
    ...
    tmpl = r"""
<div class="ph"><h1>My Profile</h1><p>View your details, payment status, and account settings.</p></div>
<div style="max-width:680px;margin:auto;">
  ...
  <div class="card"><h2>🔐 Change Password</h2>
    ...
    return render(tmpl, _title="My Profile — Civision Society", member=member, payments=payments,
                  club_balance=club_balance, nav_logo=get_nav_logo())

@app.route("/member/photo/upload", methods=["POST"])
def member_photo_upload():
    mid = session.get("member_id")
    if not mid: flash("Please log in first.","e"); return redirect(url_for("member_login"))
    cur_pw = request.form.get("current_password","").strip()
    member = get_member_by_id(mid)
    if not member: flash("Member not found.","e"); return redirect(url_for("member_login"))
...
<div class="card" style="max-width:600px;background:linear-gradient(135deg,#1a3a55,#2c3e50);color:#fff;">
...
<div class="card" style="max-width:600px;background:linear-gradient(135deg,#7d6608,#b8860b);color:#fff;">
...
<div class="card" style="max-width:600px;"><h2>Broadcast Email</h2>
